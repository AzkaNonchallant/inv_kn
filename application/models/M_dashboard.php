...
class M_dashboard extends CI_Model
{
    ...
    public function tot_barang_masuk($hariini)
    {
        ...
        $sql = "SELECT count(id_pb) tot_barang_masuk FROM tb_lab_pemeriksaan_bahan WHERE tgl='$hariini' AND is_deleted = 0";
    ...
    {
        // $kode_user = $this->kode_user();
        $sql = "SELECT count(id_barang) tot_barang FROM tb_prc_barang WHERE is_deleted = 0";
        return $this->db->query($sql);
    }
    public function tot_supplier()
    {
        // $kode_user = $this->kode_user();
        $sql = "SELECT count(id_supplier) tot_supplier FROM tb_prc_supplier WHERE is_deleted = 0";
        return $this->db->query($sql);
    }
