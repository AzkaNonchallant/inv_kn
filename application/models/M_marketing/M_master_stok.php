...
class M_master_stok extends CI_Model {
    ...
    public function update($id, $data) {
        try {
            ...
            $data['updated_by'] = $this->session->userdata('id_user') ? $this->session->userdata('id_user') : 1;
            return $this->db->where('id_master_stok_size', $id)->update($this->table, $data);
        } catch (Exception $e) {
    ...
    public function delete($id) {
        try {
            $data = [
                ...
                'updated_by' => $this->session->userdata('id_user') ? $this->session->userdata('id_user') : 1
            ];
            return $this->db->where('id_master_stok_size', $id)->update($this->table, $data);
        } catch (Exception $e) {
