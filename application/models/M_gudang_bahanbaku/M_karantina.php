...
class M_karantina extends CI_Model
{
    ...
    public function get($id = null)
    {
        ...
        $sql = "
            SELECT a.*,b.nama_supplier,c.kode_barang,c.nama_barang,c.satuan,c.qty_pack FROM tb_lab_pemeriksaan_bahan a
            LEFT JOIN tb_prc_supplier b ON a.id_supplier = b.id_supplier
    ...
    public function add($data)
    {
        ...
        $sql = "
        INSERT INTO `tb_lab_pemeriksaan_bahan`(`no_batch`,`no_surat_jalan`, `tgl`, `id_barang`, `id_supplier`, `nama_operator`, `dok_pendukung`, `jenis_kemasan`, `jml_kemasan`, `tutup`, `wadah`, `label`, `qty`,`exp`, `mfg`,`status`,`created_at`, `created_by`, `updated_at`, `updated_by`, `is_deleted`) 
    ...
    public function delete($data)
    {
        ...
        $sql = "
        DELETE FROM `tb_lab_pemeriksaan_bahan`
         WHERE `id_pb`='$data[id_pb]'
        ";
        return $this->db->query($sql);
    }
    ...
    {
        $sql = "
            SELECT a.*,b.nama_supplier,c.kode_barang,c.nama_barang,c.satuan FROM tb_lab_pemeriksaan_bahan a
            LEFT JOIN tb_prc_supplier b ON a.id_supplier = b.id_supplier
            LEFT JOIN tb_prc_barang c ON a.id_barang = c.id_barang
