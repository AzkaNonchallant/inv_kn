...
class M_laporan_barang_keluar extends CI_Model
{
    ...
    public function get($tgl = null, $tgl2 = null, $name = null)
    {
        ...
        $sql = "
            SELECT a.*,b.nama_barang,b.satuan,b.qty_pack,d.exp,d.mfg,d.no_surat_jalan,d.op_gudang,d.dok_pendukung,d.jenis_kemasan,d.jml_kemasan,d.ditolak_kemasan,d.tutup,d.wadah,d.label,d.ditolak_qty,e.status FROM tb_gbb_permintaan_barang a
            LEFT JOIN tb_prc_barang b ON a.id_barang = b.id_barang
            LEFT JOIN tb_prc_supplier c ON a.id_supplier = c.id_supplier
            LEFT JOIN tb_gbb_barang_masuk d ON a.id_barang_masuk = d.id_barang_masuk
    ...
    {
        $sql = "
            SELECT a.*,b.nama_barang FROM tb_gbb_permintaan_barang a
            LEFT JOIN tb_prc_barang b ON a.id_barang = b.id_barang
    ...
    {
        $sql = "
            SELECT a.*,c.nama_barang,c.satuan,d.nama_supplier FROM tb_gbb_barang_keluar a
            LEFT JOIN tb_gbb_barang_masuk b ON a.no_batch = b.no_batch
            LEFT JOIN tb_prc_barang c ON b.id_barang = c.id_barang
            LEFT JOIN tb_prc_supplier d ON b.id_supplier = d.id_supplier
    ...
    public function add_barang_keluar($data)
    {
        $nama_operator = $this->nama_operator();
        $sql = "
        INSERT INTO `tb_gbb_barang_keluar`(`no_batch`, `no_transfer_slip`, `qty`, `created_at`, `created_by`, `updated_at`, `updated_by`, `is_deleted`)
    ...
    {
        $sql = "
            SELECT sum(qty) tot_barang_keluar FROM `tb_gbb_barang_keluar` WHERE no_batch='$data[no_batch]' AND is_deleted = 0";
        return $this->db->query($sql);
    }
    public function jml_barang_keluar2($data)
    {
        $sql = "
            SELECT sum(a.qty) tot_barang_keluar FROM `tb_gbb_barang_keluar` a 
            LEFT JOIN tb_gbb_barang_masuk b ON a.no_batch = b.no_batch 
