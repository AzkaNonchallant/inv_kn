...
class Sk_dpb extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('M_stock_keeper/M_sk_dpb');
    ...
    public function approve()
    {
        ...
        if ($no_ppb_accounting && $approval_level) {
            $update = $this->M_sk_dpb->approve($no_ppb_accounting, $approval_level);
    ...
    public function update()
    {
        ...
        $respon = $this->M_sk_dpb->update($data);

        if ($respon) {
            redirect('Stock_keeper/Sk_dpb?alert=success&msg=Selamat anda berhasil meng-update Barang');
        } else {
            redirect('Stock_keeper/Sk_dpb?alert=error&msg=Maaf anda gagal meng-update Barang');
        }
    }
    ...
    public function index()
    {
        ...
        $data['fil_barang'] = $this->M_sk_dpb->get_filter_brng();
        $data['res_barang'] = $this->M_sk_dpb->get_master_barang();
 

        
        $data['result'] = $this->M_sk_dpb->get()->result_array();
        $data['jabatan'] = $this->M_sk_dpb->get()->result_array();
        $data['pm'] = $this->M_prc_ppb_masterbarang->get()->result_array();
        $data['name'] = 'Nama Barang yang Terpilih';

        // Load tampilan
        $this->template->load('template', 'content/stock_keeper/sk_dpb', $data);
    }
        ...
        if ($respon) {
            ...
            if (is_array($data['jumlah']) && count($data['jumlah']) > 0) {
                for ($i = 0; $i < count($data['jumlah']); $i++) {
                    ...
                    $this->M_sk_dpb->add_permintaan_barang($d);
                    // $this->M_sk_dpb->update_status_ppb($data['id_accounting_ppb'], "Approved");
                }
            } else {
                log_message('error', 'Jumlah barang tidak valid atau kosong');
            }

            redirect('Stock_keeper/Sk_dpb?alert=success&msg=Selamat anda berhasil menambah Permintaan Barang Melting');
        } else {
            redirect('Stock_keeper/Sk_dpb?alert=error&msg=Maaf anda gagal menambah Permintaan Barang Melting');
        }
    }
    ...
    public function get_barang_detail()
    {
        ...
        // Ambil detail barang
        $data = $this->M_sk_dpb->get_barang_detail($kode_barang);
    ...
    public function delete($no_ppb_accounting)
    {
        ...
        $respon = $this->M_sk_dpb->delete($data);

        if ($respon) {
            header('location:' . base_url('Stock_keeper/Sk_dpb') . '?alert=success&msg=Selamat anda berhasil menghapus Permintaan Barang Melting');
        } else {
            header('location:' . base_url('Stock_keeper/Sk_dpb') . '?alert=error&msg=Maaf anda gagal menghapus Permintaan Barang Melting');
        }
    ...
    { 
        ...
        $result = $this->M_sk_dpb->data_ppb_barang($no_ppb_accounting)->result_array();
    ...
    public function pdf_cetak_dpb($no_dpb = null)
{
    ...
    $data['detail'] = $this->M_sk_dpb->ambil_label($no_dpb)->result_array();
