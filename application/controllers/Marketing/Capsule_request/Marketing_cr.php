...
class Marketing_cr extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['result'] = $this->M_capsule_request->get1()->result_array();
		...
		$this->template->load('template', 'content/marketing/capsule_request/mcr_data', $data);
		// print_r($data);

	}

    public function data_mcr()
    {
        $no_mcr = $this->input->post('no_mcr', TRUE);

        $result = $this->M_capsule_request->data_mcr($no_mcr)->result_array();
	...
	public function add()
	{
		...
		if ($respon) {
			...
			header('location:' . base_url('Marketing/Capsule_request/Marketing_cr') . '?alert=success&msg=Selamat anda berhasil menambah Schedule Marketing');
		} else {
			header('location:' . base_url('Marketing/Capsule_request/Marketing_cr') . '?alert=error&msg=Maaf anda gagal menambah Schedule Marketing');
		}
	}
	...
	public function delete($no_mcr)
	{
		...
		if ($respon) {
			header('location:' . base_url('Marketing/Capsule_request/Marketing_cr') . '?alert=success&msg=Selamat anda berhasil menghapus Capsule Request');
		} else {
			header('location:' . base_url('Marketing/Capsule_request/Marketing_cr') . '?alert=error&msg=Maaf anda gagal menghapus Capsule Request');
		}
	}

	public function delete_cr($id_cr)
	{
		$data['id_cr'] = $id_cr;
		$respon = $this->M_capsule_request->delete($data);
